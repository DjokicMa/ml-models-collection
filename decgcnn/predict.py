...
parser.add_argument('humanpath', help='path to the directory of human descriptors.')
...
def main():
    ...
    cif_path = args.cifpath
    descriptor_path = args.humanpath
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        if args.cuda:
            input_var = (Variable(input[0].cuda(async=True), volatile=True),
                         ...
                         Variable(input[2].cuda(async=True), volatile=True),
                         input[3].cuda(async=True),
                         [crys_idx.cuda(async=True) for crys_idx in input[4]])
        else:
