...
def prepare_line_graph_batch(
    batch: Tuple[Tuple[dgl.DGLGraph, dgl.DGLGraph, torch.Tensor], torch.Tensor],
    device=None,
    non_blocking=False,
):
    ...
    batch = (
        (
            ...
            des.to(device, non_blocking=non_blocking),
        ),
        t.to(device, non_blocking=non_blocking),
    )
...
class StructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.descriptors = self.df["features"]
        self.labels = torch.tensor(self.df[target]).type(
            torch.get_default_dtype()
        )

        self.descriptors = torch.tensor(self.df["features"]).type(
            torch.get_default_dtype()
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.line_graph:
            return g, self.line_graphs[idx], descriptor, label

        return g, descriptor, label
    ...
    @staticmethod
    def collate(samples: List[Tuple[dgl.DGLGraph, torch.Tensor, torch.Tensor]]):
    ...
    def collate_line_graph(
        ...
        """Dataloader helper to batch graphs cross `samples`."""
        graphs, line_graphs, descriptors, labels = map(list, zip(*samples))
#        print (np.shape(descriptors), descriptors[0].size())
        batched_graph = dgl.batch(graphs)
        batched_line_graph = dgl.batch(line_graphs)
        batched_descriptors = torch.vstack(descriptors)
        if len(labels[0].size()) > 0:
            return batched_graph, batched_line_graph, torch.stack(labels)
        else:
            return batched_graph, batched_line_graph, batched_descriptors, torch.tensor(labels)
