...
from dealignn import models
from dealignn.data import get_train_val_loaders
from dealignn.config import TrainingConfig
from dealignn.models.alignn import ALIGNN
from dealignn.models.alignn_layernorm import ALIGNN as ALIGNN_LN
from dealignn.models.modified_cgcnn import CGCNN
from dealignn.models.dense_alignn import DenseALIGNN
from dealignn.models.densegcn import DenseGCN
from dealignn.models.icgcnn import iCGCNN
from dealignn.models.alignn_cgcnn import ACGCNN
...
def train_dgl(
    ...
    if model is None:
        # revise here to add features length
    ...
    if (
        ...
        with torch.no_grad():
            ...
            n = 0
            for dat, id in zip(test_loader, ids):
                g, lg, descriptors, target = dat
                out_data = net([g.to(device), lg.to(device), descriptors.to(device)])
                out_data = out_data.cpu().numpy().tolist()
                if config.standard_scalar_and_pca:
                    sc = pk.load(open(os.path.join(tmp_output_dir, "sc.pkl"), "rb"))
                ...
                # Fix: Handle both single and multiple targets properly
                if not isinstance(target, list):
                    target = [target]
                if not isinstance(out_data, list):
                    out_data = [out_data]

                # Ensure target and out_data have same length
                target_len = len(target)
                out_data_len = len(out_data)

                for l in range(max(target_len, out_data_len)):
                    target_val = target[min(l, target_len - 1)]
                    pred_val = out_data[min(l, out_data_len - 1)]
                    id_val = ids[min(l + n * target_len, len(ids) - 1)]

                    f.write("%s, %6f, %6f\n" % (id_val, target_val, pred_val))
        ...
        print(
            "Test MAE:", mean_absolute_error(np.array(targets), np.array(predictions))
        )
